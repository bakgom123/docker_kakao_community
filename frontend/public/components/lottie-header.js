...
class LottieHeader extends HTMLElement {
   ...
   async loadProfileImage() {
       ...
       try {
           const response = await fetch(
               `http://15.164.50.17/api/user/profile-image/${email}`,
               {
                   headers: {
   ...
   renderHeader(imageUrl) {
       this.innerHTML = `
           <div class="header">
               <a href="posts.html" id="back-button" class="back-button" style="position: absolute; left: 20px; top: 50%; transform: translateY(-50%);">
