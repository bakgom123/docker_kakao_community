...
async function fetchUserProfile(email) {
    try {
        const response = await fetch(`http://15.164.50.17/api/user/profile-image/${email}`, {
           headers: {
...
async function toggleLike(postId) {
    ...
    try {
        ...
        const response = await fetch(
            ...
            `http://15.164.50.17/api/likes/${postId}`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
...
async function checkLikeStatus(postId) {
    ...
    try {
        const response = await fetch(
            ...
            `http://15.164.50.17/api/likes/check/${postId}?email=${email}`,
            {
                headers: {
                    'Content-Type': 'application/json',
                },
...
async function fetchPost(postId) {
    try {
        ...
        await fetch(`http://15.164.50.17/api/views/${postId}`, {
            method: 'POST',
        });

        // 게시글 데이터 요청
        // const response = await fetch(`http://localhost:3000/posts/${postId}`);
        const response = await fetch(`http://15.164.50.17/api/posts/${postId}`);
        ...
        // );
        const profileResponse = await fetch(
            `http://15.164.50.17/api/user/profile-image/${data.post.author_email}`
        );
...
async function submitComment(postId) {
    ...
    try {
        ...
        const response = await fetch('http://15.164.50.17/api/comments', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function deleteComment(commentId) {
    try {
        ...
        //     {
            const response = await fetch(`http://15.164.50.17/api/comments/${commentId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
...
async function fetchCommentCount(postId) {
    try {
        ...
        // );
        const response = await fetch(`http://15.164.50.17/api/comments/${postId}`);
